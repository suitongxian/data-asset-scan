...
import { api, type ScanTask as ApiScanTask, type ParamsChanged, ScanProgressEvent } from '@/services/api'
...
const loadScanStatus = async () => {
  try {
    ...
    hasCompletedFirstScan.value = !!status.lastScanTime

    // 如果正在扫描且没有进度信息，尝试获取进度
    if (status.isScanning && !currentProgress.value) {
...
async function subscribeScanProgress() {
  ...
  try {
    ...
    // 触发新的扫描以获取进度
    // 注意：这里使用日常盘点模式来连接现有的扫描
    const es = api.triggerScan({ scanMode: 'DAILY_CHECK' })
    ...
    es.onmessage = function onmessage(event) {
      ...
      if (data.type === 'progress') {
        currentProgress.value = {
          ...
          taskPhase: data.phase || 'scanning',
          fileTotal: data.totalCount,
          fileScannedCount: data.scannedCount
        }
      } else if (data.type === 'complete' && data.success) {
        isScanning.value = false
        currentProgress.value = null
        loadScanTasks().catch((e) => console.error('加载任务失败:', e))
        loadScanStatus().catch((e) => console.error('加载状态失败:', e))
        es.close()
      } else if (data.type === 'error' || (data.type === 'complete' && !data.success)) {
        isScanning.value = false
        currentProgress.value = null
        loadScanStatus().catch((e) => console.error('加载状态失败:', e))
        es.close()
    ...
    es.onerror = function onerror() {
      console.error('扫描进度连接错误')
      es.close()
    }

    eventSource = es
  } catch (error) {
    console.error('订阅扫描进度失败:', error)
  }
}
